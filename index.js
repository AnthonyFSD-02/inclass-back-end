...
"@cluster0.duyqd6w.mongodb.net/myFirstDB?retryWrites=true&w=majority",
    {
...
*/
app.post("/users", async (req, res) => { const username = req.body.username; const password = req.body.password; const user = {
    username: username,
    password: password,
  };
...
/* An API get request using URL path
...
app.put("/users", async (req, res) => { const password = req.body.password; const username = req.body.username; const user = {
    username: username,
    password: password,
  };
const results = await userModel.replaceOne({ username: username }, user);
console.log("matched: " + results.matchedCount); console.log("modified: " + results.modifiedCount); res.send(results);
});
...
app.post("/users/register", async (request, response) => {
    ...
    try {
        ...
        const user = await userModel.findOne({ username: username });
        if (user) {
            const msg = "Invalid registration - username " + username + " already exists." 
            console.log(msg);
            ...
            const msg = "Username is not Alphanumeric or Password is not strong enough"
            response.send({ success: false, msg: msg});
            return;
        }
    } catch (error) {   
